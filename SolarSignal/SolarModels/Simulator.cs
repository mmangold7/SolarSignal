...
namespace SolarSignal.SolarModels
{
    public class Simulator
    {
        ...
        public void DecreaseFuturesCalculations()
        {
            ...
            if (AmountOfFuturePositionsToGenerate < 0)
            {
                AmountOfFuturePositionsToGenerate = 0;
            }
        }
        ...
        public void SetUpSol()
        {
            ...
            //make the moon
            var moon = this.CreateCircularOrbiterOf(earth, 15, .1f, 1, "white", "moon");

            //make some other planets
            var mercury = this.CreateCircularOrbiterOf(sun, 100, 100, 3, "orange", "mercury");
            var venus = this.CreateCircularOrbiterOf(sun, 200, 1000, 5, "yellow", "venus");
            var mars = this.CreateCircularOrbiterOf(sun, 400, 500, 4, "red", "mars");
        }

        public void ResetSimulation()
        {
            Bodies.RemoveAll(b => b.GetType() != typeof(Player) && b.GetType() != typeof(Missile));
        ...
        private void HandleCollisions(Body body)
        {
            ...
            foreach (var otherBody in collisionEligibleBodies)
            {
                ...
                if (distance < sumOfRadii && !body.CollidingWithBodies.Contains(otherBody))
                {
                    ...
                    var bodyFinalVelocity = (initialMomentum + otherBody.Mass * (otherBody.Velocity - body.Velocity)) / totalMass;
                    var otherBodyFinalVelocity = (initialMomentum + body.Mass * (body.Velocity - otherBody.Velocity)) / totalMass;
        ...
        {
            var damageEligibleMissiles = Missiles.Where(m => Vector2.Distance(player.Position, m.Position) < player.Radius * 2 + m.Radius && m.ParentBody != player);
        ...
        private void HandlePlayerInput()
        {
            ...
            if (Players.TrueForAll(p => !p.Input.DownPressed && !p.Input.UpPressed && !p.FuturesIncremented && !p.FuturesDecremented && !p.Input.ShootPressed) && _calculatedAtLeastOneFuture)
            {
            ...
            foreach (var player in Players)
            {
                ...
                //ship acceleration
                var scaleMagnitude = 3 / 30f * (Convert.ToInt32(player.Input.UpPressed) - Convert.ToInt32(player.Input.DownPressed));
